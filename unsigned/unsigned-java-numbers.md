...
Определённую конкуренцию может составить библиотека jOOU. Во-первых, это 18 Кб против 3 Мб. Во-вторых, у Guava нет классов-обёрток для типов byte и short. И в-третьих, в jOOU используется комбинация классов обёрток с переменными большего разряда и с кэшированием, что должно себя великолепно проявить с byte. Последнее. Если есть возможность избежать работы с типами вроде Short - избегайте. Если не получается, то подключайте OPC Foundation UA JAVA Legacy.
